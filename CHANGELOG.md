...
### Fixed
- **Мёртвый remote-домен** — зона `blysspeak.space` перестала резолвиться, из-за чего `tf register`, `tf link`, `tf sync`, авто-синк демона и SVG-карточка в README молча уходили в никуда. Публичный инстанс не принимал события с 2026-07-04. Все дефолты переведены на `timeforged.nexalix.io` — тот же демон, валидный сертификат (2aab68b)
- **CORS** — origins демона обновлены на `nexalix.io` / `timeforged.nexalix.io`

## [0.5.3] — 2026-05-23

### Fixed
- **Windows: без окна консоли** — демон и трей больше не открывают чёрное окно при запуске (cc321b5)
- **Установщик** — убрана scheduled task синхронизации, спамившая всплывающими окнами (184b4c5)

## [0.5.2] — 2026-04-01

### Added
- **Двусторонний sync** — `tf sync` теперь не только пушит локальные события, но и подтягивает события с remote-сервера (44b33b2)
- **Установщик** — на шаге remote-синка предлагает выбор: завести новый аккаунт или привязаться к существующему; heartbeat-хук Claude Code стал кроссплатформенным (5a2d46f)

### Fixed
- Добавлена недостающая зависимость `windows-sys` в `timeforged-tray` — трей не собирался на Windows (274b762)

## [0.5.1] — 2026-04-01

### Added
- **`tf link <remote_key>`** — привязка машины к существующему remote-аккаунту одной командой (проверяет подключение, сохраняет конфиг, запускает первый sync)
